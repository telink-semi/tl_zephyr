...
volatile uint32_t start_rf_tick;        /*  start rf tick */
...
_attribute_ram_code_sec_ uint8_t rf_rx_process(rf_packet_t *p_rf_data)
{
	if (device_status == STATE_PAIRING) /*  pair status */
	{
		...
		if (((pair_ack_dat_ptr->cmd == PAIR_ACK_CMD) ||
		     (pair_ack_dat_ptr->cmd == RECONNECT_ACK_CMD)) &&
		    (pair_ack_dat_ptr->did == p_pair_dat->did)) /*  ack and same did */
		{
	...
	} else if (device_status == STATE_NORMAL) /*  normal status */
	{
		...
		if ((keyboard_send_need_f == 2)) /*  mouse ack */
		{
			...
			{
				kb_led_status =
					km_ack_dat_ptr->host_led_status; /*  read led status */
...
{
	rf_state = RF_IDLE_STATUS;
	static uint32_t ack_miss_no; /*  ack miss no */
 ...
 * @note
 */
_attribute_ram_code_sec_ void d24g_rf_loop(void) /*  rf state machine loop */
{
	uint8_t *ptr = 0;
	...
	if (rf_state == RF_IDLE_STATUS) /*  rf status idle */
	{
		...
		} else if (device_status == STATE_NORMAL) /*  status normal */
		{
			...
			set_data_access_code(
				flash_dev_info.dongle_id); /*  dongle id to be data access code */
			...
			if (keyboard_send_need_f == 0) {
				...
				if (p) {
					 ...
					 * km_dat->key_type, km_dat->km_dat[0], km_dat->km_dat[1],
					 * km_dat->km_dat[2], km_dat->km_dat[3], km_dat->km_dat[4],
					 * km_dat->km_dat[5], km_dat->km_dat[6], km_dat->km_dat[7]);
					 */
	...
	if (device_status == STATE_NORMAL) {
		if (has_new_key_event) {
			...
			if ((idle_count < 3) || key_buf_24g.cnt || 1) {
				...
				if (p == 0) {
					pp_fifo_push(&tx_fifo, NORMAL_KB_DATA_CMD,
						     &key_buf_24g.nk[0], 8);
				}
			}
		}
