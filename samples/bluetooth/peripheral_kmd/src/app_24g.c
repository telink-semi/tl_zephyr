...
_attribute_ram_code_sec_ void tlk_d25f_to_n22_mode_info(kb_mode_t mode_flag)
{
    ...
    fence_iorw;
    tlk_mailbox_send_data(TLK_MESSAGE_FROM_D25F_TO_N22_MODE,cmd);
...
//          if(!tlk_d25f_hci_send_message(TLK_SHARE_MEMORY_MESSAGE_TYPE_BLE, TxBuf, sizeof(p24g_evt_t) + p_evt->len)) {
...
_attribute_ram_code_sec_ uint8_t p24g_send_sm_msg(uint8_t type, uint8_t op, uint8_t *data, uint8_t len)
{
    ...
    if (!tlk_d25f_hci_send_message(TLK_SHARE_MEMORY_MESSAGE_TYPE_2P4G, TxBuf, sizeof(p24g_evt_t) + p_evt->len))
    {
        //         if(!tlk_n22_sync_send_message(TLK_SHARE_MEMORY_MESSAGE_TYPE_BLE, TxBuf, sizeof(p24g_evt_t) + p_evt->len)) {
        tlkapi_printf(1, "d25f send sm message success\n");
        printk("d25f send sm message success %x %x\n", type, op);
    }
    else
    {
        tlkapi_printf(1, "d25f send sm message fail\n");
        printk("d25f send sm message faillll\n");
...
//    irq_enable();
    ...
    tlk_mailbox_register_message_cb(TLK_MESSAGE_FROM_N22_TO_D25F_KM_DATA, app_2p4g_mb_km_data_cb);

    tlk_d25f_register_hci_receive_cb(TLK_SHARE_MEMORY_MESSAGE_TYPE_2P4G, app_2p4g_d25f_sm_rx_cb);
    ...
    tlk_mailbox_send_data(TLK_MESSAGE_FROM_D25F_TO_N22_2P4G_KB_TX_ADDRESS, cmd);
    ...
    // tlk_mailbox_send_data(TLK_MESSAGE_FROM_D25F_TO_N22_2P4G_SPP_TX_ADDRESS, cmd);
