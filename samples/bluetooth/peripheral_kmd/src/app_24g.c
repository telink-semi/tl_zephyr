/** @file
 *  @brief app_24g.c- Application main entry point
 */
...
static inline void app_wdt_init()
{
	...
	/**
	 * Wd_clear() must be executed before each call to wd_start() to avoid abnormal watchdog
	 * reset time because the initial count value is not 0. For example, the watchdog is reset
	 * soon or a few minutes later.
	 */
	wd_clear();
	wd_start();
}
...
_attribute_ram_code_sec_ static void app_2p4g_set_power_state(p24g_device_status_e state,
							      uint32_t tick)
{
	/* if ((state == STATE_TO_IDLE) && (g_power_state != STATE_TO_SLEEP)) {
	 *     g_power_state = STATE_TO_IDLE;
	 *     if (tick) {
	 *         app_timer1_start((tick - stimer_get_tick() - APP_WFI_WAKEUP_EARLY_TIME_US *
...
static _attribute_ram_code_sec_ void app_2p4g_packet_enqueue(uint8_t *data, uint16_t len)
{
	...
	if (p_evt->type == P24G_MB_CMD_USB) {
		...
		case P24G_USB_OP_EP_WRITE:
			...
			usb0hw_write_ep_data((p_pkt->ep_addr & (~USB0_DIR_IN_MASK)), p_pkt->data,
					     p_pkt->len);
...
_attribute_ram_code_sec_ void event_test(void)
{
	...
	if (clock_time_exceed(tick, 2000000)) {
		...
		if (!mcc_d25f_hci_send_msg(TxBuf, sizeof(p24g_evt_t) + p_evt->len)) {
			/* if (!tlk_n22_sync_send_message(TLK_SHARE_MEMORY_MESSAGE_TYPE_BLE, TxBuf,
			 *       sizeof(p24g_evt_t) + p_evt->len)) {
			 */
...
_attribute_ram_code_sec_ uint8_t p24g_send_sm_msg(uint8_t type, uint8_t op, uint8_t *data,
						  uint8_t len)
{
	static uint8_t TxBuf[64] = {0}; /* [!!important] */
...
_attribute_ram_code_sec_ void app_2p4g_d25f_sm_rx_cb(uint8_t *data, uint32_t len)
{
	...
	if (cmd < P24G_SM_CMD_MAX && p24g_cmd_table[cmd]) {
		...
		tlkapi_send_string_data(APP_LOG_EN, "error! unknown sm command", data, len);
	}
}

_attribute_ram_code_sec_ uint8_t p24g_send_spp_data(uint8_t cmd, unsigned char *data,
						    unsigned char len)
{
...
uint8_t p24g_register_sm_cmd_handler(p24g_sm_cmd_e cmd, p24g_sm_cmd_handler_t handler)
{
	...
	if (cmd < P24G_SM_CMD_MAX && handler) {
		...
		tlkapi_printf(APP_LOG_EN, "%s error %x\n", __func__, cmd);
	}
...
_attribute_ram_code_ void p24g_user_init_deepRetn(void)
{
	...
	/***
	 * TL321X(buteo):
	 *   PLL_192M_CCLK_96M_HCLK_48M_PCLK_24M_MSPI_48M, the time from retention_reset(.s file) to
	 * here is 550us. PLL_192M_CCLK_32M_HCLK_32M_PCLK_32M_MSPI_48M, the time from
	 * retention_reset(.s file) to here is 578us
	 */
	DBG_CHN0_HIGH;
	irq_enable();
...
_attribute_ram_code_sec_ static void app_2p4g_handle_save_pairing_info(uint8_t *data, uint16_t len)
{
	...
	if (p_evt->type == P24G_SM_CMD_SAVE_PAIR_INFO) {
		...
		if (flash_dev_info.side_id != side_id) {
			...
			/* save_data_to_flash(flash_sector_2p4_inf, sizeof(ST_FLASH_DEV_INFO), */
			/* (unsigned char *)&flash_dev_info.side_id, (int *)&dev_info_idx); */

			int ret = nvs_write(&user_fs, APP_2P4G_PAIR_INFO_ID,
					    (unsigned char *)&flash_dev_info.side_id,
					    sizeof(ST_FLASH_DEV_INFO));
...
_attribute_ram_code_sec_ static void app_2p4g_save_report_rate_info(uint8_t rr)
{
	if ((rr == REPORT_RATE_8K) || (rr == REPORT_RATE_125)) {
		...
		if (flash_dev_other_info.side_id != side_id) {
			...
			tlkapi_send_string_data(APP_LOG_EN, "saving other info",
						&flash_dev_other_info.side_id, 5);

			/* save_data_to_flash(flash_sector_2p4_other_inf, */
			/* sizeof(ST_FLASH_DEV_OTHER_INFO), (unsigned char */
			/* *)&flash_dev_other_info.side_id, (int *)&dev_other_info_idx); */
			int ret = nvs_write(&user_fs, APP_2P4G_APP_INFO_ID,
					    (unsigned char *)&flash_dev_other_info.side_id,
					    sizeof(ST_FLASH_DEV_OTHER_INFO));
...
static void app_p24g_send_info_2_n22(void)
{
	...
	if (dev_info_idx >= 0) {
		p24g_send_sm_msg(P24G_SM_CMD_MISC, P24G_SM_OP_MISC_PEER_INFO,
				 flash_dev_info.peer_addr, MAC_ADDR_LEN);
		p24g_enable_reconn(true);
	}
	...
	if (dev_other_info_idx >= 0) {
		p24g_send_sm_msg(P24G_SM_CMD_MISC, P24G_SM_OP_MISC_REPORT_RATE,
				 &flash_dev_other_info.report_rate, 1);
	}
}
...
#if APP_DELAY_ENTER_SUSPEND_TIME_US
			if (stick && tick1_exceed_tick2(stimer_get_tick(),
							stick + APP_DELAY_ENTER_SUSPEND_TIME_US *
									SYSTEM_TIMER_TICK_1US)) {
				stick = 0;
#endif
				...
				if (tick1_exceed_tick2(g_wakeup_stick,
						       stimer_get_tick() +
							       APP_SUSPEND_LIMIT_MIN_TIME_US *
								       SYSTEM_TIMER_TICK_1US)) {
					/* gpio_set_level(GPIO_PH2, 0); */
					/* app_proc_before_sleep(); */
					/* DBG_GPIO_TOGGLE(APP_IO_EN,  GPIO_PD7); */
					if (tick1_exceed_tick2(
						    g_wakeup_stick,
						    stimer_get_tick() +
							    APP_SUSPEND_LONG_TIME_MIN_US *
								    SYSTEM_TIMER_TICK_1US)) {
					...
					pm_sleep_wakeup(
						SUSPEND_MODE, PM_WAKEUP_TIMER, PM_TICK_STIMER,
						g_wakeup_stick - (APP_WAKEUP_EARLY_TIME_US +
								  (short_suspend_flag ? 0 : 600)) *
									 SYSTEM_TIMER_TICK_1US);
					...
					if (!short_suspend_flag) {
						/* gpio_set_level(GPIO_PH2, 0); */
						pm_set_dig_module_power_switch(FLD_PD_ZB_EN,
									       PM_POWER_UP);
						p24g_send_sm_msg(P24G_SM_CMD_MISC,
								 P24G_SM_OP_MISC_LONG_SUSP_RET, 0,
								 0);
					} else {
						p24g_send_sm_msg(P24G_SM_CMD_MISC,
								 P24G_SM_OP_MISC_SUSP_RET, 0, 0);
					}
					/* gpio_set_level(GPIO_PH2, 1); */
