...
extern uint8_t g_2p4g_state_active;
...
/* Define the Flash storage partition used by NVS */
...
/* The size of the NVS sector needs to match the erase page size of the Flash */
#define NVS_SECTOR_SIZE (4096)
/* The number of NVS sectors */
...
static void p24g_pairing_info_check(void)
{
    ...
    if (ret == -ENOENT) {
        LOG_INF("NVS APP_2P4G_PAIR_INFO_ID naver saved\n");
...
_attribute_ram_code_sec_ void app_proc_before_sleep(void)
{
    #if DIGIT_KEYSCAN_FUN_ENABL
    digit_keyscan_disable();
    #endif
}

_attribute_ram_code_sec_ void app_proc_after_sleep(void)
{
    #if DIGIT_KEYSCAN_FUN_ENABL
    digit_keyscan_enable();
    #endif
}
...
_attribute_ram_code_sec_ void app_clock_init(app_clock_config_e select)
{
    ...
    switch (select) 
    {
        ...
        case CLOCK_CONFIG_1V_48_48:
            clock_init(CLK_BASEBAND_PLL_144M, CLK_DIV3, CCLK_DIV1_TO_HCLK_DIV4_TO_PCLK, CLK_DIV12); // 48M 48M 12M 12M
            pm_set_dig_ldo(DIG_VOL_1V_MODE, 1000);
            analog_write_reg8(0x29, 0x03);//1.26|1.15
            analog_write_reg8(0x27, 0x15);//1.00
            break;
        ...
        case CLOCK_CONFIG_1V_48_24:
            clock_init(CLK_BASEBAND_PLL_192M, CLK_DIV4, CCLK_DIV2_TO_HCLK_DIV4_TO_PCLK, CLK_DIV12); // 48M 24M 12M 16M
            pm_set_dig_ldo(DIG_VOL_1V_MODE, 1000); //1.0
            analog_write_reg8(0x29, 0x03);//1.15
            analog_write_reg8(0x27, 0x15);//1.00
            break;
...
static int peripheral_comm_init(void)
{
    ...
    if (ret == -ENOENT) {
        LOG_INF("NVS USER_STORAGE_APP_INFO_ID naver saved\n");
    ...
    if (ret != 0) {
        LOG_INF("Error: failed to configure mode_2p4_pin io \n");
    }
    ...
    if (ret != 0) {
        LOG_INF("Error: failed to configure mode_ble_pin io \n");
    }
...
static void soc_tlx_mcc_init(void)
{
 ...
 _attribute_ram_code_sec_ void public_loop(void)
{
    ...
    if (loop_cnt > 15) { // (15 * 3)ms
        ...
        if (k_uptime_get_32() - last_time > 5000)//5s
        {
            ...
            LOG_INF("mast_id %d \n", ble_app_pip_info.mast_id);
            LOG_INF("usb_connected_ok %d \n", usb_connected_ok);
        }

        if (BIT_IS_SET(user_active_disconnect, BLE_SWITCH_PIPE)) { 
            LOG_INF("disconnect in loop \r\n");
        ...
        if (fun_mode != last_fun_mode) {
            LOG_INF("fun_mode change %d \n", fun_mode);
            pp_fifo_reset(&tx_fifo);
            pp_fifo_reset(&d25fKbTxFifo);

            if (fun_mode == KB_MODE_2P4G){
                if (last_fun_mode == KB_MODE_USB) {
                    LOG_INF("usb mode_exit enter 2p4g mode\r\n");
                    // app_usb_mode_exit();
                    sys_reboot(SYS_REBOOT_COLD);
                } else if (last_fun_mode == KB_MODE_BLE) {
                    LOG_INF("ble_mode_exit enter 2p4g mode\r\n");
                    // ble_mode_exit();
                    sys_reboot(SYS_REBOOT_COLD);
                }
            } else if (fun_mode == KB_MODE_BLE){
                if (last_fun_mode == KB_MODE_USB) {
                    LOG_INF("app_usb_mode_exit  enter ble mode\r\n");
                    // app_usb_mode_exit();
                    //ble_init();
                    sys_reboot(SYS_REBOOT_COLD);
                } else if (last_fun_mode == KB_MODE_2P4G) {
                    LOG_INF("app_usb_mode_exit enter 2p4g mode\r\n");
                    // app_2p4g_mode_exit();
                    sys_reboot(SYS_REBOOT_COLD);
                }
                // ble_mode_enter();
                ...
                if (last_fun_mode == KB_MODE_2P4G) {
                    LOG_INF("2p4g mode exit enter usb mode\r\n");
                    // app_2p4g_mode_exit();
                    sys_reboot(SYS_REBOOT_COLD);
                } else if (last_fun_mode == KB_MODE_BLE) {
                    LOG_INF("ble mode exit enter usb mode\r\n");
                     //ble_mode_exit();
                     //bt_disable();
                     sys_reboot(SYS_REBOOT_COLD);
                }
