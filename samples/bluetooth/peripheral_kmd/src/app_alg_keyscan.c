...
/* the minimum buffer size (unit:byte), adc data format 16bits */
...
unsigned char get_key_value(unsigned short *buf, unsigned short release_threshold,
			    unsigned short press_threshold)
{
	...
	for (int col = 0; col < TOTAL_COL; col++) {
		for (int i = 0; i < 8; i++) {
			...
			if (map_window[col][i] != 0) {
				...
				LOG_INF("adc_buffer[%d] = %d", 8 * col + i,
					adc_buffer[8 * col + i]);
			}
			if (press_flag == 1) {
				hw_now_bits[col] |= (1 << i);
