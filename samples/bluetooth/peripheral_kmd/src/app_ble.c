...
static void connected(struct bt_conn *conn, uint8_t err)
{
	...
	LOG_INF("bt_conn_le_param_update %d\n", ret);
}
			     ...
			     enum bt_security_err err)
{
	...
	if (!err) {
		...
		if (level >= BT_SECURITY_L2) {
			LOG_INF("Encryption is complete and sufficient.\\n");
...
{
	LOG_INF("LE conn param updated: int 0x%04x lat %d to %d", interval, latency, timeout);
...
static void bt_ready(int err)
{
	...
	LOG_INF("Bluetooth initialized id %d\n", adv_param.id);
...
void start_change_ble_pipe_by_delay_work(void)
{
	...
	if (ret != 1) {
		LOG_INF("Failed to schedule work. Error code: %d\n", ret);
		return;
	}
}
...
void start_pairing_by_delay_work(void)
{
	...
	if (ret != 1) {
		LOG_INF("Failed to schedule pariring work. Error code: %d\n", ret);
		return;
	}
}
...
void ble_start_pairing_delayed_work_handler(struct k_work *work)
{
    ...
    LOG_INF("ble_start_pairing_delayed_work_handler mast id %d\r\n", ble_app_pip_info.mast_id);

	err = bt_le_adv_stop();
	if (err) {
		LOG_INF("Advertising failed to stop (err %d)\n", err);
		return;
	}
...
void disconnect_current_connection(void) 
{
    if (connected_handle) {
        ...
        if (err) {
            LOG_INF("disconnect error: %d\n", err);
        } else {
            LOG_INF("disconnect send\n");
        }
    }
}
