...
int kbd_set_report(const struct device *dev, struct usb_setup_packet *setup, int32_t *len,
			uint8_t **data)
{
	gpio_pin_set(led1.port, led1.pin, (**data & HID_KBD_LED_CAPS_LOCK));
...
static void timer1_isr(const struct gpio_dt_spec *led)
{
...
int main(void)
{
	...
	riscv_plic_set_priority(IRQ_GPIO_IRQ3, 1);
