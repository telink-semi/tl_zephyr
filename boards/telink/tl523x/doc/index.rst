...
The TL5238X Generic Starter Kit is a hardware platform which
...
The TL5238X SoC integrates a powerful 32-bit RISC-V MCU, DSP, 2.4 GHz ISM Radio, 128 KB SRAM
...
.. figure:: img/tl5238_block_diagram.png
     :align: center
     :alt: TL5238X_SOC

The TL5238X default board configuration provides the following hardware components:
...
The Zephyr TL5238X board configuration supports the following hardware features:
...
The TL5238X board is configured to use the 24 MHz external crystal oscillator
with the on-chip PLL/DIV generating the 96 MHz system clock.
The following values also could be assigned to the system clock in the board DTS file
(``boards/riscv/tl323x/tl5238x-common.dtsi``):
...
The TL5238X SoC has five GPIO controllers (PORT_A to PORT_F), and the next are
currently enabled:
...
``boards/riscv/tl323x/tl5238x-common.dtsi`` file:
...
The TL5238X SoC has 1 UART. The Zephyr console output is assigned to UART0.
...
.. code-block:: console
   ...
   west build -b tl5238x samples/hello_world
...
To flash the TL5238X board see the sources below:
