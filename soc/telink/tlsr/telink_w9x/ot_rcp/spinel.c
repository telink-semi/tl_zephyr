	...
	for (; *fmt != 0; fmt = spinel_next_packed_datatype(fmt)) {
		...
		case SPINEL_DATATYPE_DATA_C: {
			...
			if ((fmt[0] == SPINEL_DATATYPE_DATA_WLEN_C) ||
				...
				int size_len = spinel_datatype_pack(tx_cb, ctx, SPINEL_DATATYPE_UINT16_S, data_size_arg);
	...
	for (; *fmt != 0; fmt = spinel_next_packed_datatype(fmt)) {
		...
		case SPINEL_DATATYPE_IPv6ADDR_C: {
			...
			if (copy_buff) {
				...
				const struct spinel_ipv6addr **arg_ptr = va_arg(*args,
					const struct spinel_ipv6addr **);
		...
		case SPINEL_DATATYPE_EUI64_C: {
			...
			if (copy_buff) {
				...
				const struct spinel_eui64 **arg_ptr = va_arg(*args, const struct spinel_eui64 **);
		...
		case SPINEL_DATATYPE_EUI48_C: {
			...
			if (copy_buff) {
				...
				const struct spinel_eui48 **arg_ptr = va_arg(*args, const struct spinel_eui48 **);
		...
		case SPINEL_DATATYPE_UINT_PACKED_C: {
			...
			ret += decoded_size;
			data_in += decoded_size;
			data_len -= decoded_size;
			break;
		}
		...
		case SPINEL_DATATYPE_DATA_WLEN_C: {
			...
			if ((fmt[0] == SPINEL_DATATYPE_DATA_WLEN_C) ||
				...
				pui_len = spinel_datatype_unpack(data_in, data_len, copy_buff,
					SPINEL_DATATYPE_UINT16_S, &block_len);
			...
			if (copy_buff) {
				if (!block_len_ptr || *block_len_ptr < block_len) {
					return -EINVAL;
				}
