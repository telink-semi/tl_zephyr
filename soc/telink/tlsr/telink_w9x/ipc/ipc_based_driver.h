...
enum ipc_dispatcher_id {
	...
	IPC_OPUS_REMOTE = 0x1000,
	IPC_DISPATCHER_PM = 0x1100,
...
int ipc_based_driver_send(struct ipc_based_driver *drv, const void *tx_data, size_t tx_len,
...
#define IPC_DISPATCHER_MK_ID(ipc, inst) (((ipc & 0xffffff) << 8) | (inst & 0xff))
...
#define IPC_DISPATCHER_UNPACK_FUNC_ONLY_WITH_ERROR_PARAM(name)                                     \
	static void unpack_##name(void *unpack_data, const uint8_t *pack_data,                     \
...
#define IPC_DISPATCHER_HOST_SEND_DATA(ipc, inst, name, tx_buff, rx_buff, timeout_ms)               \
	do {                                                                                       \
		...
		struct ipc_based_driver_ctx ctx = {                                                \
			...
			.result = rx_buff,                                                         \
                                                                                                   ...
                                                                                                   \
		int ipc_err =                                                                      \
			ipc_based_driver_send(ipc, packed_data, packed_len, &ctx, timeout_ms);     \
                                                                                                   \
		if (ipc_err) {                                                                     \
			assert(0);                                                                 \
		}                                                                                  \
...
#define IPC_DISPATCHER_HOST_SEND_DATA_BY_HEAP(ipc, inst, name, tx_buff, rx_buff, timeout_ms)       \
	do {                                                                                       \
