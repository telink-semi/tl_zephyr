...
static int openthread_rcp_work_q_init(void)
{
	struct k_work_queue_config cfg = {
		.name = "ot_rcp_workq", .no_yield = false};
					...
					k_ticks_t *start_time)
{
	...
	if (result >= 0) {
		*start_time = sys_clock_timeout_end_calc(
			K_MSEC(CONFIG_TELINK_OT_SPINEL_RESPONSE_TIMEOUT_MS));
	}
					   ...
					   struct openthread_rcp_buffer *data_income)
{
	...
	k_ticks_t now = sys_clock_tick_get();
	if (now < start_time) {
		timeout.ticks = start_time - now;
	}
	if (!k_msgq_get(&ot_rcp->spinel_msgq, data_income, timeout)) {
