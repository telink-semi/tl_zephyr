...
static int wdt_tlx_disable(const struct device *dev)
{
	ARG_UNUSED(dev);
...
static int wdt_tlx_install_timeout(const struct device *dev,
...
static int wdt_tlx_feed(const struct device *dev, int channel_id)
{
	ARG_UNUSED(dev);
...
static const struct wdt_driver_api wdt_tlx_api = {
	.setup = wdt_tlx_setup,
	.disable = wdt_tlx_disable,
	.install_timeout = wdt_tlx_install_timeout,
	.feed = wdt_tlx_feed,
};


static int wdt_tlx_init(const struct device *dev)
{
	ARG_UNUSED(dev);
...
DEVICE_DT_INST_DEFINE(0, wdt_tlx_init, NULL,
	NULL, NULL, PRE_KERNEL_1,
	CONFIG_KERNEL_INIT_PRIORITY_DEVICE, &wdt_tlx_api);
