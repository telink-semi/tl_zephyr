...
#if CONFIG_WATCHDOG_AUTO
static struct k_timer b9x_WdtTimer;

void b9x_WdtTimerTimeoutCallback(struct k_timer *timer)
{
	if (!timer) {
		return;
	}
...
static int wdt_b9x_setup(const struct device *dev, uint8_t options)
{
	...
	wd_start();
#if CONFIG_WATCHDOG_AUTO
	k_timer_stop(&b9x_WdtTimer);
	k_timer_init(&b9x_WdtTimer, &b9x_WdtTimerTimeoutCallback, NULL);
	k_timer_start(&b9x_WdtTimer, K_MSEC(CONFIG_TELINK_WDT_FEED_TIME),
