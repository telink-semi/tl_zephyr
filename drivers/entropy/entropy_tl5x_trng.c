...
static int entropy_tl5x_trng_get_entropy(const struct device *dev,
...
static int entropy_tl5x_trng_get_entropy_isr(const struct device *dev,
					      uint8_t *buffer, uint16_t length,
					      uint32_t flags)
{
	ARG_UNUSED(dev);
...
DEVICE_DT_INST_DEFINE(0, entropy_tl5x_trng_init,
		      NULL, NULL, NULL,
		      PRE_KERNEL_1, CONFIG_ENTROPY_INIT_PRIORITY,
		      &entropy_tl5x_trng_api);
