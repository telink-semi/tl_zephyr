...
/* Driver dts compatibility: telink,tl521x_uart */
#define DT_DRV_COMPAT telink_tl521x_uart

/* Get UART instance */
#define GET_UART(dev)                                                                              \
	((volatile struct uart_tl521x_t *)((const struct uart_tl521x_config *)dev->config) \
		->uart_addr)
...
/* tl521x UART registers structure */
struct __packed uart_tl521x_t {
...
/* tl521x UART data structure */
struct uart_tl521x_data {
...
/* tl521x UART config structure */
struct uart_tl521x_config {
...
/* Get tx fifo count */
static inline uint8_t uart_tl521x_get_tx_bufcnt(volatile struct uart_tl521x_t *uart)
{
	return (uart->bufcnt & FLD_UART_TX_BUF_CNT) >> FLD_UART_TX_BUF_CNT_OFFSET;
}

/* Get rx fifo count */
static inline uint8_t uart_tl521x_get_rx_bufcnt(volatile struct uart_tl521x_t *uart)
{
	return (uart->bufcnt & FLD_UART_RX_BUF_CNT) >> FLD_UART_RX_BUF_CNT_OFFSET;
}

/* Check for prime */
static uint8_t uart_tl521x_is_prime(uint32_t n)
{
	uint32_t i = 5;
...
static void uart_tl521x_cal_div_and_bwpc(uint32_t baudrate, uint32_t pclk, uint16_t *divider,
				      uint8_t *bwpc)
{
	uint8_t i = 0, j = 0;
	uint32_t primeInt = 0;
	...
	if (uart_tl521x_is_prime(primeInt)) {
		primeInt += 1;
	} else if (primeDec > 5) {
		primeInt += 1;
		if (uart_tl521x_is_prime(primeInt)) {
...
static void uart_tl521x_init(volatile struct uart_tl521x_t *uart, uint16_t divider, uint8_t bwpc,
...
static void uart_tl521x_irq_handler(const struct device *dev)
{
#ifndef CONFIG_UART_INTERRUPT_DRIVEN
	ARG_UNUSED(dev);
#else
	struct uart_tl521x_data *data = dev->data;
...
static int uart_tl521x_configure(const struct device *dev, const struct uart_config *cfg)
{
	struct uart_tl521x_data *data = dev->data;
	uint16_t divider;
	uint8_t bwpc;
	...
	volatile struct uart_tl521x_t *uart = GET_UART(dev);
	...
	/* UART configure */
	uart_tl521x_cal_div_and_bwpc(cfg->baudrate, sys_clk.pclk * 1000 * 1000, &divider, &bwpc);
	uart_tl521x_init(uart, divider, bwpc, parity, stop_bits);
...
static int uart_tl521x_config_get(const struct device *dev, struct uart_config *cfg)
{
	struct uart_tl521x_data *data = dev->data;
...
static int uart_tl521x_driver_init(const struct device *dev)
{
	...
	uart_tl521x_cal_div_and_bwpc(cfg->baud_rate, sys_clk.pclk * 1000 * 1000, &divider, &bwpc);
	uart_tl521x_init(uart, divider, bwpc, UART_PARITY_NONE, UART_STOP_BIT_1);
...
static void uart_tl521x_poll_out(const struct device *dev, uint8_t c)
{
	volatile struct uart_tl521x_t *uart = GET_UART(dev);
	struct uart_tl521x_data *data = dev->data;

	while (uart_tl521x_get_tx_bufcnt(uart) >= UART_TX_BUF_CNT) {
	}
...
static int uart_tl521x_poll_in(const struct device *dev, unsigned char *c)
{
	volatile struct uart_tl521x_t *uart = GET_UART(dev);
	struct uart_tl521x_data *data = dev->data;

	if (uart_tl521x_get_rx_bufcnt(uart) == 0) {
		return -1;
	}
...
static int uart_tl521x_err_check(const struct device *dev)
{
	volatile struct uart_tl521x_t *uart = GET_UART(dev);
...
static int uart_tl521x_fifo_fill(const struct device *dev, const uint8_t *tx_data, int size)
{
	int i = 0;
	volatile struct uart_tl521x_t *uart = GET_UART(dev);
	struct uart_tl521x_data *data = dev->data;
...
static int uart_tl521x_fifo_read(const struct device *dev, uint8_t *rx_data, const int size)
{
	int rx_count;
	volatile struct uart_tl521x_t *uart = GET_UART(dev);
	struct uart_tl521x_data *data = dev->data;
...
static void uart_tl521x_irq_tx_enable(const struct device *dev)
{
	volatile struct uart_tl521x_t *uart = GET_UART(dev);
...
static void uart_tl521x_irq_tx_disable(const struct device *dev)
{
	volatile struct uart_tl521x_t *uart = GET_UART(dev);
...
static int uart_tl521x_irq_tx_ready(const struct device *dev)
{
	volatile struct uart_tl521x_t *uart = GET_UART(dev);

	return ((uart_tl521x_get_tx_bufcnt(uart) < UART_TX_BUF_CNT) &&
...
static int uart_tl521x_irq_tx_complete(const struct device *dev)
{
	volatile struct uart_tl521x_t *uart = GET_UART(dev);

	return (uart_tl521x_get_tx_bufcnt(uart) == 0) ? 1 : 0;
}

/* API implementation: irq_rx_enable */
static void uart_tl521x_irq_rx_enable(const struct device *dev)
{
	volatile struct uart_tl521x_t *uart = GET_UART(dev);
...
static void uart_tl521x_irq_rx_disable(const struct device *dev)
{
	volatile struct uart_tl521x_t *uart = GET_UART(dev);
...
static int uart_tl521x_irq_rx_ready(const struct device *dev)
{
	volatile struct uart_tl521x_t *uart = GET_UART(dev);

	return (uart_tl521x_get_rx_bufcnt(uart) > 0) ? 1 : 0;
}

/* API implementation: irq_err_enable */
static void uart_tl521x_irq_err_enable(const struct device *dev)
{
	volatile struct uart_tl521x_t *uart = GET_UART(dev);
...
static void uart_tl521x_irq_err_disable(const struct device *dev)
{
	volatile struct uart_tl521x_t *uart = GET_UART(dev);
...
static int uart_tl521x_irq_is_pending(const struct device *dev)
{
	volatile struct uart_tl521x_t *uart = GET_UART(dev);
...
static int uart_tl521x_irq_update(const struct device *dev)
{
	ARG_UNUSED(dev);
...
static void uart_tl521x_irq_callback_set(const struct device *dev, uart_irq_callback_user_data_t cb,
				      void *cb_data)
{
	struct uart_tl521x_data *data = dev->data;
...
int uart_tl521x_drv_cmd(const struct device *dev, uint32_t cmd, uint32_t p)
{
	int result = -ENOTSUP;
		...
		for (uint8_t i = 0; i < pcfg->state_cnt; i++) {
			...
			if (state->id == PINCTRL_STATE_DEFAULT) {
				for (uint8_t j = 0; j < state->pin_cnt; j++) {
					...
					bool level = gpio_get_level(TL521X_PINMUX_GET_PIN(*pin));
...
__GENERIC_SECTION(.ram_code)
static int uart_tl521x_pm_action(const struct device *dev, enum pm_device_action action)
{
	volatile struct uart_tl521x_t *uart = GET_UART(dev);
	struct uart_tl521x_data *data = dev->data;
...
static const struct uart_driver_api uart_tl521x_driver_api = {
	.poll_in = uart_tl521x_poll_in,
	.poll_out = uart_tl521x_poll_out,
	.err_check = uart_tl521x_err_check,
#ifdef CONFIG_UART_USE_RUNTIME_CONFIGURE
	.configure = uart_tl521x_configure,
	.config_get = uart_tl521x_config_get,
#endif
#ifdef CONFIG_UART_INTERRUPT_DRIVEN
	.fifo_fill = uart_tl521x_fifo_fill,
	.fifo_read = uart_tl521x_fifo_read,
	.irq_tx_enable = uart_tl521x_irq_tx_enable,
	.irq_tx_disable = uart_tl521x_irq_tx_disable,
	.irq_tx_ready = uart_tl521x_irq_tx_ready,
	.irq_tx_complete = uart_tl521x_irq_tx_complete,
	.irq_rx_enable = uart_tl521x_irq_rx_enable,
	.irq_rx_disable = uart_tl521x_irq_rx_disable,
	.irq_rx_ready = uart_tl521x_irq_rx_ready,
	.irq_err_enable = uart_tl521x_irq_err_enable,
	.irq_err_disable = uart_tl521x_irq_err_disable,
	.irq_is_pending = uart_tl521x_irq_is_pending,
	.irq_update = uart_tl521x_irq_update,
	.irq_callback_set = uart_tl521x_irq_callback_set,
#endif
#ifdef CONFIG_UART_DRV_CMD
	.drv_cmd = uart_tl521x_drv_cmd,
#endif
};

#define UART_TL521X_INIT(n) \
	PM_DEVICE_DT_INST_DEFINE(n, uart_tl521x_pm_action); \
	static void uart_tl521x_irq_connect_##n(void); \
	PINCTRL_DT_INST_DEFINE(n); \
	static const struct uart_tl521x_config uart_tl521x_cfg_##n = { \
		.uart_addr = DT_INST_REG_ADDR(n), \
		.baud_rate = DT_INST_PROP(n, current_speed), \
		.pcfg = PINCTRL_DT_INST_DEV_CONFIG_GET(n), \
		.pirq_connect = uart_tl521x_irq_connect_##n, \
		.hw_flow_control = DT_INST_PROP(n, hw_flow_control)}; \
	static struct uart_tl521x_data uart_tl521x_data_##n; \
	DEVICE_DT_INST_DEFINE(n, uart_tl521x_driver_init, PM_DEVICE_DT_INST_GET(n), \
			      &uart_tl521x_data_##n, &uart_tl521x_cfg_##n, PRE_KERNEL_1, \
			      CONFIG_SERIAL_INIT_PRIORITY, (void *)&uart_tl521x_driver_api); \
	static void uart_tl521x_irq_connect_##n(void) \
	{ \
		IRQ_CONNECT(DT_INST_IRQN(n), DT_INST_IRQ(n, priority), uart_tl521x_irq_handler, \
			    DEVICE_DT_INST_GET(n), 0); \
		riscv_plic_irq_enable(DT_INST_IRQN(n)); \
		riscv_plic_set_priority(DT_INST_IRQN(n), DT_INST_IRQ(n, priority)); \
	}

DT_INST_FOREACH_STATUS_OKAY(UART_TL521X_INIT)
