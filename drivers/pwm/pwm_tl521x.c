...
#define DT_DRV_COMPAT telink_tl521x_pwm
...
struct pwm_tl521x_channel_state {
	uint32_t period;
	uint32_t pulse;
	pwm_flags_t flags;
};

struct pwm_tl521x_config {
...
struct pwm_tl521x_data {
	uint8_t out_pin_ch_connected;
#ifdef CONFIG_PM_DEVICE
	struct pwm_tl521x_channel_state ch[CONFIG_PWM_TELINK_TL521X_MAX_CHANNELS];
#endif /* CONFIG_PM_DEVICE */
};

/* API implementation: init */
static int pwm_tl521x_init(const struct device *dev)
{
	const struct pwm_tl521x_config *config = dev->config;
	uint32_t pwm_clk_div;

	/* Check maximum number of PWM channels */
	if (config->channels > CONFIG_PWM_TELINK_TL521X_MAX_CHANNELS) {
		return -EINVAL;
	}
...
static int pwm_tl521x_set_cycles(const struct device *dev, uint32_t channel,
				 uint32_t period_cycles, uint32_t pulse_cycles,
				 pwm_flags_t flags)
{
	struct pwm_tl521x_data *data = dev->data;
	const struct pwm_tl521x_config *config = dev->config;
...
static int pwm_tl521x_get_cycles_per_sec(const struct device *dev,
...
#ifdef CONFIG_PM_DEVICE
static int pwm_tl521x_pm_action(const struct device *dev, enum pm_device_action action)
{
	const struct pwm_tl521x_config *config = dev->config;
	struct pwm_tl521x_data *data = dev->data;
	...
	case PM_DEVICE_ACTION_RESUME:
		...
		for (uint8_t channel = 0; channel < config->channels; channel++) {
			if (data->out_pin_ch_connected & BIT(channel)) {
				pwm_tl521x_set_cycles(dev, channel, data->ch[channel].period,
...
#if CONFIG_SOC_SERIES_RISCV_TELINK_TL521X_RETENTION
	{
		for (uint8_t channel = 0; channel < config->channels; channel++) {
			if (data->out_pin_ch_connected & BIT(channel)) {
				uint32_t pin = TL521X_PINMUX_GET_PIN(config->pins[channel]);
...
static const struct pwm_driver_api pwm_tl521x_driver_api = {
	.set_cycles = pwm_tl521x_set_cycles,
	.get_cycles_per_sec = pwm_tl521x_get_cycles_per_sec,
};

/* PWM driver registration */
#define PWM_TL521X_INIT(n)                                                 \
                                                                           \
	PM_DEVICE_DT_INST_DEFINE(n, pwm_tl521x_pm_action);                     \
                                                                           \
	static const pinctrl_soc_pin_t pwm_tl521x_pins##n[] = {                \
		...
		(UINT32_MAX,))                                                  \
                                                                           ...
                                                                           \
	static const struct pwm_tl521x_config config##n = {                    \
		.pins = pwm_tl521x_pins##n,                                        \
		...
		.clk32k_ch_enable = (                                           \
			...
			(DT_INST_PROP(n, clk32k_ch5_enable) << 5U)                  \
                                                                           ...
                                                                           \
	static struct pwm_tl521x_data data##n;                                 \
                                                                           \
	DEVICE_DT_INST_DEFINE(n, pwm_tl521x_init,                              \
		PM_DEVICE_DT_INST_GET(n), &data##n, &config##n,                 \
		POST_KERNEL, CONFIG_PWM_INIT_PRIORITY,                          \
		&pwm_tl521x_driver_api);

DT_INST_FOREACH_STATUS_OKAY(PWM_TL521X_INIT);
