...
#define DT_DRV_COMPAT telink_tl521x_adc
...
#include <zephyr/dt-bindings/adc/tl521x-adc.h>

/* Zephyr Logging headers */
#include <zephyr/logging/log.h>
LOG_MODULE_REGISTER(adc_tl521x, CONFIG_ADC_LOG_LEVEL);
...
struct tl521x_adc_data {
	...
	/* To save resources, the ADC thread can be disabled because some user is not using */
	K_KERNEL_STACK_MEMBER(stack, CONFIG_ADC_TL521X_ACQUISITION_THREAD_STACK_SIZE);
};

/* Driver configuration structure */
struct tl521x_adc_cfg {
	uint8_t resolution;
...
#ifdef CONFIG_PM_DEVICE
struct adc_channel_cfg tl521x_channel_cfg;
#endif /* CONFIG_PM_DEVICE */

/* Number of samples used to calculate the average. */
#define SD_ADC_SAMPLE_CNT 16

/* Convert dts pin to tl521x SDK pin */
static sd_adc_p_input_pin_def_e adc_tl521x_get_p_pin(uint8_t dt_pin)
{
	sd_adc_p_input_pin_def_e adc_pin;

	switch (dt_pin) {
	case DT_ADC_GPIO_PC0:
		adc_pin = SD_ADC_GPIO_PC0P;
		break;
	case DT_ADC_GPIO_PC1:
		adc_pin = SD_ADC_GPIO_PC1P;
		break;
	case DT_ADC_GPIO_PC2:
		adc_pin = SD_ADC_GPIO_PC2P;
		break;
	case DT_ADC_GPIO_PC3:
		adc_pin = SD_ADC_GPIO_PC3P;
		break;
	case DT_ADC_GPIO_PC4:
		adc_pin = SD_ADC_GPIO_PC4P;
		break;
	case DT_ADC_GPIO_PC5:
		adc_pin = SD_ADC_GPIO_PC5P;
		break;
...
static int adc_tl521x_setup_sample_freq(uint32_t sample_freq, sd_adc_sample_clk_freq_e *sample_clk)
{
	switch (sample_freq) {
	case 1000000:
...
static int adc_tl521x_setup_downsample_rate(uint32_t downsample_rate,
					    sd_adc_downsample_rate_e *rate)
{
 ...
 */
static int sd_adc_collect_and_calculate_average(struct tl521x_adc_data *data)
{
	...
	for (int i = SD_ADC_SAMPLE_CNT >> 2; i < (SD_ADC_SAMPLE_CNT - (SD_ADC_SAMPLE_CNT >> 2));
	     i++) {
...
{
	struct tl521x_adc_data *data = CONTAINER_OF(ctx, struct tl521x_adc_data, ctx);
...
static void adc_tl521x_acquisition_thread(const struct device *dev)
{
	struct tl521x_adc_data *data = dev->data;
...
static int adc_tl521x_validate_buffer_size(const struct adc_sequence *sequence)
{
	struct tl521x_adc_data data;
...
static int adc_tl521x_validate_sequence(const struct adc_sequence *sequence)
{
	int status;
	...
	status = adc_tl521x_validate_buffer_size(sequence);
	if (status) {
...
static int adc_tl521x_channel_setup(const struct device *dev,
				    const struct adc_channel_cfg *channel_cfg)
{
	const struct tl521x_adc_cfg *cfg = dev->config;
	...
	ret = adc_tl521x_setup_sample_freq(cfg->sample_freq, &sample_clk);
	if (ret != 0) {
		return ret;
	}

	/* Configure downsample rate */
	ret = adc_tl521x_setup_downsample_rate(cfg->downsample_rate, &downsample_rate);
	if (ret != 0) {
		return ret;
	}
	...
	if (channel_cfg->input_positive == DT_ADC_VBAT) {
		...
		sd_adc_p_input_pin_def_e p_pin = adc_tl521x_get_p_pin(channel_cfg->input_positive);
		sd_adc_n_input_pin_def_e n_pin = SD_ADC_GNDN;

		/* Configure GPIO sampling parameters */
		sd_adc_gpio_cfg_t adc_gpio_cfg = {.clk_freq = sample_clk,
...
#ifdef CONFIG_PM_DEVICE
	memcpy(&tl521x_channel_cfg, channel_cfg, sizeof(struct adc_channel_cfg));
#endif
	return 0;
}

/* API implementation: read */
static int adc_tl521x_read(const struct device *dev, const struct adc_sequence *sequence)
{
	struct tl521x_adc_data *data = dev->data;
	int status;

	/* Verification parameters */
	status = adc_tl521x_validate_sequence(sequence);
	if (status != 0) {
		return status;
	}
	/* Save buffer pointer */
...
static int adc_tl521x_init(const struct device *dev)
{
	struct tl521x_adc_data *data = dev->data;
	const struct tl521x_adc_cfg *config = dev->config;
	int err;
	...
	/* To save resources, the ADC thread can be disabled because some user is not using */
	k_thread_create(&data->thread, data->stack, CONFIG_ADC_TL521X_ACQUISITION_THREAD_STACK_SIZE,
			(k_thread_entry_t)adc_tl521x_acquisition_thread, (void *)dev, NULL, NULL,
			CONFIG_ADC_TL521X_ACQUISITION_THREAD_PRIO, 0, K_NO_WAIT);
...
static int adc_tl521x_pm_action(const struct device *dev, enum pm_device_action action)
{
	switch (action) {
...
PM_DEVICE_DT_INST_DEFINE(0, adc_tl521x_pm_action);
#endif /* CONFIG_PM_DEVICE */

static struct tl521x_adc_data tl521x_adc_data_0 = {
	ADC_CONTEXT_INIT_TIMER(tl521x_adc_data_0, ctx),
	ADC_CONTEXT_INIT_LOCK(tl521x_adc_data_0, ctx),
	ADC_CONTEXT_INIT_SYNC(tl521x_adc_data_0, ctx),
};
...
static const struct tl521x_adc_cfg tl521x_adc_cfg_0 = {
...
static const struct adc_driver_api adc_tl521x_api = {
	.channel_setup = adc_tl521x_channel_setup,
	.read = adc_tl521x_read,
};

DEVICE_DT_INST_DEFINE(0, adc_tl521x_init, PM_DEVICE_DT_INST_GET(0), &tl521x_adc_data_0,
		      &tl521x_adc_cfg_0, POST_KERNEL, CONFIG_ADC_INIT_PRIORITY, &adc_tl521x_api);
