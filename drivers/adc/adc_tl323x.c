...
struct tl323x_adc_data {
	...
	K_THREAD_STACK_MEMBER(stack, CONFIG_ADC_TL323X_ACQUISITION_THREAD_STACK_SIZE);
};
...
static int adc_tl323x_setup_sample_freq(uint32_t sample_freq, sd_adc_sample_clk_freq_e *sample_clk)
{
	...
	case 2000000:
		*sample_clk = SD_ADC_SAPMPLE_CLK_2M;
		return 0;
	default:
		LOG_ERR("Selected sample frequency %lu is not supported", sample_freq);
		return -EINVAL;
	}
}
...
static int adc_tl323x_setup_downsample_rate(uint32_t downsample_rate, sd_adc_downsample_rate_e *rate)
{
	...
	case 256:
		*rate = SD_ADC_DOWNSAMPLE_RATE_256;
		return 0;
	default:
		LOG_ERR("Selected downsample rate %lu is not supported", downsample_rate);
		return -EINVAL;
	}
}
