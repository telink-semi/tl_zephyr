...
static int adc_tl323x_setup_downsample_rate(uint32_t downsample_rate,
...
static int adc_tl323x_init(const struct device *dev)
{
	...
	/* To save resources, the ADC thread can be disabled because some user is not using */
	k_thread_create(&data->thread, data->stack, CONFIG_ADC_TL323X_ACQUISITION_THREAD_STACK_SIZE,
...
DEVICE_DT_INST_DEFINE(0, adc_tl323x_init, PM_DEVICE_DT_INST_GET(0), &tl323x_adc_data_0,
		      &tl323x_adc_cfg_0, POST_KERNEL, CONFIG_ADC_INIT_PRIORITY, &adc_tl323x_api);
