...
#define DT_DRV_COMPAT telink_tl521x_zb

#define LOG_MODULE_NAME ieee802154_tl521x
...
#include "ieee802154_tl521x.h"

#include "ieee802154_tl521x_frame.c"
...
#if defined(CONFIG_IEEE802154_TL521X_MAC_FLASH)
#include <zephyr/drivers/flash.h>
#include <zephyr/storage/flash_map.h>

static const struct device *flash_device = DEVICE_DT_GET(DT_CHOSEN(zephyr_flash_controller));
#endif /* CONFIG_IEEE802154_TL521X_MAC_FLASH */

#ifdef CONFIG_IEEE802154_TL521X_BLE_COEXIST
#include "debug_gpio.h"
K_SEM_DEFINE(ieee802154_task_ready_sem, 0, 1);
/* define 802.15.4 TX sending status type */
#define TL521X_RF_ZIGBEE_TX_IS_IDLE    0
#define TL521X_RF_ZIGBEE_TX_IS_SENDING 1
#define TL521X_RF_ZIGBEE_TX_IS_STOPPED 2
/* FIX 802.15.4 TX sending issue */
static volatile uint8_t tl521x_rf_zigbee_tx_is_sending;
...
#endif /*CONFIG_IEEE802154_TL521X_BLE_COEXIST*/

#ifdef CONFIG_OPENTHREAD_FTD
/* TLX radio source match table structure */
static struct tl521x_src_match_table src_match_table;
#endif /* CONFIG_OPENTHREAD_FTD */

#if !defined(CONFIG_OPENTHREAD_THREAD_VERSION_1_1)
/* TLX radio ACK table structure */
static struct tl521x_enh_ack_table enh_ack_table;

/* mac keys data */
static struct tl521x_mac_keys mac_keys;
#endif
#if CONFIG_SOC_SERIES_RISCV_TELINK_TL521X_RETENTION
__attribute__((section(".bss"))) uint8_t rxpkt_buffer[TL521X_TRX_LENGTH] = {0};
__attribute__((section(".bss"))) uint8_t txpkt_buffer[TL521X_TRX_LENGTH] = {0};
#endif /* CONFIG_SOC_SERIES_RISCV_TELINK_TL521X_RETENTION */
/* TLX data structure */
static struct tl521x_data data = {
#ifdef CONFIG_OPENTHREAD_FTD
...
#if defined CONFIG_IEEE802154_TL521X_OPTIMIZATION && CONFIG_IEEE802154_TL521X_OPTIMIZATION
...
ALWAYS_INLINE static void tl521x_src_match_table_clean(struct tl521x_src_match_table *table)
{
	memset(table, 0, sizeof(struct tl521x_src_match_table));
}

/* Search in radio search match table */
ALWAYS_INLINE static bool tl521x_src_match_table_search(const struct tl521x_src_match_table *table,
...
ALWAYS_INLINE static void tl521x_src_match_table_add(struct tl521x_src_match_table *table,
						  const uint8_t *addr, bool ext)
{
	if (!tl521x_src_match_table_search(table, addr, ext)) {
		for (size_t i = 0; i < 2 * CONFIG_OPENTHREAD_MAX_CHILDREN; i++) {
...
ALWAYS_INLINE static void tl521x_src_match_table_remove(struct tl521x_src_match_table *table,
						     const uint8_t *addr, bool ext)
{
	for (size_t i = 0; i < 2 * CONFIG_OPENTHREAD_MAX_CHILDREN; i++) {
		if (table->item[i].valid && table->item[i].ext == ext &&
...
ALWAYS_INLINE static void tl521x_src_match_table_remove_group(struct tl521x_src_match_table *table,
							   bool ext)
{
	for (size_t i = 0; i < 2 * CONFIG_OPENTHREAD_MAX_CHILDREN; i++) {
		if (table->item[i].valid && table->item[i].ext == ext) {
 ...
 */
ALWAYS_INLINE static bool tl521x_require_pending_bit(const struct ieee802154_frame *frame)
{
	bool result = false;
		...
		if (frame->general.type == IEEE802154_FRAME_FCF_TYPE_DATA) {
			...
			if (!frame->sec_header ||
				...
				const uint8_t *cmd_id =
					frame->payload_ie
						? tl521x_ieee802154_get_data(frame->payload,
									  frame->payload_len)
						: frame->payload;
				if (cmd_id && *cmd_id == TL521X_CMD_ID_DATA_REQ) {
					result = true;
				}
			} else {
...
/* clean radio search match table */
ALWAYS_INLINE static void tl521x_enh_ack_table_clean(struct tl521x_enh_ack_table *table)
{
	memset(table, 0, sizeof(struct tl521x_enh_ack_table));
}

/* Search in enhanced ack table */
ALWAYS_INLINE static int tl521x_enh_ack_table_search(const struct tl521x_enh_ack_table *table,
...
ALWAYS_INLINE static void tl521x_enh_ack_table_add(struct tl521x_enh_ack_table *table,
						const uint8_t *addr_short, const uint8_t *addr_ext,
						const struct ieee802154_header_ie *ie_header)
{
	int idx = tl521x_enh_ack_table_search(table, addr_short, addr_ext);
...
ALWAYS_INLINE static void tl521x_enh_ack_table_remove(struct tl521x_enh_ack_table *table,
						   const uint8_t *addr_short,
						   const uint8_t *addr_ext)
{
	for (size_t i = 0; i < CONFIG_OPENTHREAD_MAX_CHILDREN; i++) {
...
/* Clean mac keys data */
ALWAYS_INLINE static void tl521x_mac_keys_data_clean(struct tl521x_mac_keys *mac_keys_data)
{
	memset(mac_keys_data, 0, sizeof(struct tl521x_mac_keys));
}

ALWAYS_INLINE static const uint8_t *tl521x_mac_keys_get(const struct tl521x_mac_keys *mac_keys_data,
						     uint8_t key_id)
{
	const uint8_t *result = NULL;

	if (key_id) {
		for (size_t i = 0; i < TL521X_MAC_KEYS_ITEMS; i++) {
...
ALWAYS_INLINE static uint32_t tl521x_mac_keys_frame_cnt_get(
						const struct tl521x_mac_keys *mac_keys_data,
						uint8_t key_id)
{
	uint32_t result = 0;
...
ALWAYS_INLINE static void tl521x_mac_keys_frame_cnt_inc(struct tl521x_mac_keys *mac_keys_data,
						     uint8_t key_id)
{
	if (key_id) {
		for (size_t i = 0; i < TL521X_MAC_KEYS_ITEMS; i++) {
...
ALWAYS_INLINE static uint16_t tl521x_get_csl_phase(struct tl521x_data *tlx)
{
...
ALWAYS_INLINE static void tl521x_disable_pm(struct tl521x_data *tlx)
{
...
ALWAYS_INLINE static void tl521x_enable_pm(struct tl521x_data *tlx)
{
#ifdef CONFIG_PM_DEVICE
...
/* Set filter PAN ID */
ALWAYS_INLINE static int tl521x_set_pan_id(const struct device *dev, uint16_t pan_id)
{
...
ALWAYS_INLINE static int tl521x_set_short_addr(const struct device *dev, uint16_t short_addr)
{
	struct tl521x_data *tlx = dev->data;
...
ALWAYS_INLINE static int tl521x_set_ieee_addr(const struct device *dev, const uint8_t *ieee_addr)
{
	struct tl521x_data *tlx = dev->data;
...
ALWAYS_INLINE static bool tl521x_run_filter(const struct device *dev,
	...
	do {
		if (frame->dst_panid != NULL) {
			if (memcmp(frame->dst_panid, tlx->filter_pan_id,
			    ...
			    memcmp(frame->dst_panid, TL521X_BROADCAST_ADDRESS,
				   IEEE802154_FRAME_LENGTH_PANID) != 0) {
				break;
			}
		}
		if (frame->dst_addr != NULL) {
			if (frame->dst_addr_ext) {
				...
				if (memcmp(frame->dst_addr, TL521X_BROADCAST_ADDRESS,
...
/* Get MAC address */
ALWAYS_INLINE static uint8_t *tl521x_get_mac(const struct device *dev)
{
	struct tl521x_data *tlx = dev->data;

#if defined(CONFIG_IEEE802154_TL521X_MAC_RANDOM)
...
#elif defined(CONFIG_IEEE802154_TL521X_MAC_FLASH)
	(void)flash_read(flash_device,
			 FIXED_PARTITION_OFFSET(vendor_partition) +
			 IEEE802154_TL521X_FLASH_MAC_OFFSET,
			 tlx->mac_addr, IEEE802154_FRAME_LENGTH_ADDR_EXT);
#else  /* CONFIG_IEEE802154_TL521X_MAC_STATIC */
	...
	tlx->mac_addr[4] = CONFIG_IEEE802154_TL521X_MAC4;
	tlx->mac_addr[5] = CONFIG_IEEE802154_TL521X_MAC5;
	tlx->mac_addr[6] = CONFIG_IEEE802154_TL521X_MAC6;
	tlx->mac_addr[7] = CONFIG_IEEE802154_TL521X_MAC7;
#endif /* IEEE802154_TL521X_MAC_TYPE */

	return tlx->mac_addr;
}

/* Convert RSSI to LQI */
ALWAYS_INLINE static uint8_t tl521x_convert_rssi_to_lqi(int8_t rssi)
{
	uint32_t lqi32 = 0;

	/* check for MIN value */
	if (rssi < TL521X_RSSI_TO_LQI_MIN) {
		return 0;
	}

	/* convert RSSI to LQI */
	lqi32 = TL521X_RSSI_TO_LQI_SCALE * (rssi - TL521X_RSSI_TO_LQI_MIN);
...
ALWAYS_INLINE static void tl521x_update_rssi_and_lqi(const struct device *dev, struct net_pkt *pkt)
{
	struct tl521x_data *tlx = dev->data;
	int8_t rssi;
	uint8_t lqi;

	rssi = ((signed char)(tlx->rx_buffer[tlx->rx_buffer[TL521X_LENGTH_OFFSET] +
					     TL521X_RSSI_OFFSET])) -
	       110;
	lqi = tl521x_convert_rssi_to_lqi(rssi);
...
/* Prepare TX buffer */
ALWAYS_INLINE static void tl521x_set_tx_payload(const struct device *dev, uint8_t *payload,
					     uint8_t payload_len)
{
	struct tl521x_data *tlx = dev->data;
	...
	memcpy(tlx->tx_buffer + TL521X_PAYLOAD_OFFSET, payload, payload_len);
}

/* Handle acknowledge packet */
ALWAYS_INLINE static void tl521x_handle_ack(const struct device *dev, const void *buf,
					   size_t buf_len, uint64_t rx_time)
{
	struct tl521x_data *tlx = dev->data;
	...
	do {
		...
		tl521x_update_rssi_and_lqi(dev, ack_pkt);
...
ALWAYS_INLINE static void tl521x_send_ack(const struct device *dev, struct ieee802154_frame *frame)
{
	struct tl521x_data *tlx = dev->data;
	uint8_t ack_buf[64];
	size_t ack_len;
	...
	if (frame->general.ver == IEEE802154_FRAME_FCF_VER_2015) {
		if (frame->general.se_bit) {
			key = tl521x_mac_keys_get(tlx->mac_keys, 1);
			if (key) {
				tl521x_mac_keys_frame_cnt_inc(tlx->mac_keys, 1);
				frame_cnt = tl521x_mac_keys_frame_cnt_get(tlx->mac_keys, 1);
...
#ifdef CONFIG_OPENTHREAD_CSL_RECEIVER
			if (tlx->csl_period > 0) {
				(void)tl521x_ieee802154_ie_csl_commit(
					(uint8_t *)frame->payload, frame->payload_len,
					tlx->csl_period, tl521x_get_csl_phase(tlx));
			}
#endif /* CONFIG_OPENTHREAD_CSL_RECEIVER */
		}
	}
	...
	if (tl521x_ieee802154_frame_build(frame, ack_buf, sizeof(ack_buf), &ack_len)) {
#if !defined(CONFIG_OPENTHREAD_THREAD_VERSION_1_1)
		if (frame->general.se_bit) {
			if (!ieee802154_tl521x_crypto_encrypt(
...
#endif
		tl521x_set_tx_payload(dev, ack_buf, ack_len);
		unsigned int t1 = (stimer_get_tick() - t0) / SYSTEM_TIMER_TICK_1US;

		if (t1 < CONFIG_IEEE802154_TL521X_SET_TXRX_DELAY_US) {
			delay_us(CONFIG_IEEE802154_TL521X_SET_TXRX_DELAY_US - t1);
		}
...
/* RX IRQ handler */
ALWAYS_INLINE static void tl521x_rf_rx_isr(const struct device *dev)
{
	struct tl521x_data *tlx = dev->data;
	int status = -EINVAL;
	...
	do {
		...
		if (IS_ENABLED(CONFIG_IEEE802154_RAW_MODE) ||
			...
			tl521x_ieee802154_frame_parse(payload, length - TL521X_FCS_LENGTH, &frame);
		} else {
			length -= TL521X_FCS_LENGTH;
			tl521x_ieee802154_frame_parse(payload, length, &frame);
		}
		if (!frame.general.valid) {
...
#if defined(CONFIG_NET_PKT_TIMESTAMP) && defined(CONFIG_NET_PKT_TXTIME)
					tl521x_handle_ack(dev, payload, length, rx_time);
#else
					tl521x_handle_ack(dev, payload, length, 0);
#endif /* CONFIG_NET_PKT_TIMESTAMP && CONFIG_NET_PKT_TXTIME */
		...
		if (!tl521x_run_filter(dev, &frame)) {
		...
		if (frame.general.ack_req) {
#ifdef CONFIG_OPENTHREAD_FTD
			if (tl521x_require_pending_bit(&frame)) {
				if (frame.src_addr) {
					if (!tlx->src_match_table->enabled ||
					    tl521x_src_match_table_search(tlx->src_match_table,
								       frame.src_addr,
								       frame.src_addr_ext)) {
						frame_pending = true;
					}
			...
			if (enh_ack) {
				...
				int idx = tl521x_enh_ack_table_search(
					tlx->enh_ack_table,
		...
		tl521x_update_rssi_and_lqi(dev, pkt);
#if defined(CONFIG_NET_PKT_TIMESTAMP) && defined(CONFIG_NET_PKT_TXTIME)
...
/* TX IRQ handler */
ALWAYS_INLINE static void tl521x_rf_tx_isr(const struct device *dev)
{
	struct tl521x_data *tlx = dev->data;
#ifndef CONFIG_IEEE802154_TL521X_BLE_COEXIST
	...
	tl521x_rf_zigbee_tx_is_sending = TL521X_RF_ZIGBEE_TX_IS_IDLE;
#endif /* CONFIG_IEEE802154_TL521X_BLE_COEXIST */
...
/* IRQ handler */
#ifdef CONFIG_IEEE802154_TL521X_BLE_COEXIST
__GENERIC_SECTION(.ram_code) void tl521x_rf_isr(const void *parameter)
{
	(void)parameter;
...
#else
__GENERIC_SECTION(.ram_code) static void tl521x_rf_isr(const struct device *dev)
{
	if (rf_get_irq_status(FLD_RF_IRQ_RX)) {
		tl521x_rf_rx_isr(dev);
	} else if (rf_get_irq_status(FLD_RF_IRQ_TX)) {
...
volatile bool tl521x_rf_zigbee_250K_mode;

#ifdef CONFIG_IEEE802154_TL521X_BLE_COEXIST

__attribute__((noinline)) void tl521x_init_802154_rf_hw(void)
{
	...
	rf_set_tx_dma(1, TL521X_TRX_LENGTH);
	rf_set_rx_dma(tlx->rx_buffer, 0, TL521X_TRX_LENGTH);
	if (tlx->current_channel != TL521X_TX_CH_NOT_SET) {
		rf_set_chn(TL521X_LOGIC_CHANNEL_TO_PHYSICAL(tlx->current_channel));
	}
	if (tlx->current_dbm != TL521X_TX_PWR_NOT_SET) {
...
ALWAYS_INLINE static int tl521x_start_radio(struct tl521x_data *tlx)
{
	wd_32k_feed();
	tl521x_disable_pm(tlx);
...
ALWAYS_INLINE static int tl521x_start_radio(struct tl521x_data *tlx)
{
	...
	if (!tlx->is_started) {
		...
		irq_connect_dynamic(DT_INST_IRQN(0), DT_INST_IRQ(0, priority),
				    (void (*)(const void *))tl521x_rf_isr,
				    DEVICE_DT_INST_GET(0), 0);
#endif /* CONFIG_DYNAMIC_INTERRUPTS */
		if (!tl521x_rf_zigbee_250K_mode) {
#if !defined(CONFIG_OPENTHREAD_THREAD_VERSION_1_1)
			ske_dig_en();
#endif
			...
			rf_mode_init();
			rf_set_zigbee_250K_mode();
			tl521x_rf_zigbee_250K_mode = true;
		}
		rf_set_rx_maxlen(144);
		rf_set_tx_dma(1, TL521X_TRX_LENGTH);
		rf_set_rx_dma(tlx->rx_buffer, 0, TL521X_TRX_LENGTH);
...
ALWAYS_INLINE static int tl521x_stop_radio(struct tl521x_data *tlx)
{
	...
	tl521x_enable_pm(tlx);

	return 0;
}

ALWAYS_INLINE static int tl521x_set_channel_radio(struct tl521x_data *tlx, uint16_t channel)
{
	...
	if (tlx->current_channel != channel) {
		...
		if (tlx->is_started) {
			rf_set_chn(TL521X_LOGIC_CHANNEL_TO_PHYSICAL(channel));
			rf_set_rxmode();
		}
	}
	return 0;
...
static void tl521x_csl_rx_work(struct k_work *item)
{
	struct tl521x_data *tlx =
		CONTAINER_OF(k_work_delayable_from_work(item), struct tl521x_data, csl_rx_work);

	if (tlx->csl_rx_duration_us) {
		uint16_t current_channel = tlx->current_channel;

		(void)tl521x_set_channel_radio(tlx, tlx->csl_rx_channel);
		(void)tl521x_start_radio(tlx);
...
static int tl521x_init(const struct device *dev)
{
	...
	IRQ_CONNECT(DT_INST_IRQN(0), DT_INST_IRQ(0, priority), tl521x_rf_isr, DEVICE_DT_INST_GET(0),
		    0);
	...
	tlx->current_channel = TL521X_TX_CH_NOT_SET;
	tlx->current_dbm = TL521X_TX_PWR_NOT_SET;
#ifdef CONFIG_OPENTHREAD_FTD
	tl521x_src_match_table_clean(tlx->src_match_table);
	tlx->src_match_table->enabled = true;
#endif /* CONFIG_OPENTHREAD_FTD */
	tlx->event_handler = NULL;
#if !defined(CONFIG_OPENTHREAD_THREAD_VERSION_1_1)
	tl521x_enh_ack_table_clean(tlx->enh_ack_table);
	tl521x_mac_keys_data_clean(tlx->mac_keys);
#endif
#ifdef CONFIG_OPENTHREAD_CSL_RECEIVER
	k_work_init_delayable(&tlx->csl_rx_work, tl521x_csl_rx_work);
	tlx->csl_rx_duration_us = 0;
...
static void tl521x_iface_init(struct net_if *iface)
{
	const struct device *dev = net_if_get_device(iface);
	struct tl521x_data *tlx = dev->data;
	uint8_t *mac = tl521x_get_mac(dev);
...
static int tl521x_cca(const struct device *dev)
{
	ARG_UNUSED(dev);
#if defined CONFIG_IEEE802154_TL521X_OPTIMIZATION && CONFIG_IEEE802154_TL521X_OPTIMIZATION
...

	{
		...
		do {
			rssi += rf_get_rssi();
			cnt++;
		} while (!clock_time_exceed(t1, TL521X_CCA_TIME_MAX_US));
		rssi /= cnt;
		if (rssi > CONFIG_IEEE802154_TL521X_CCA_RSSI_THRESHOLD) {
			return -EBUSY;
		} else {
			return 0;
		}
...
RAM_CODE_SECTION_IEEE802154
static int tl521x_set_channel(const struct device *dev, uint16_t channel)
{
	return tl521x_set_channel_radio(dev->data, channel);
}

/* API implementation: filter */
RAM_CODE_SECTION_IEEE802154
static int tl521x_filter(const struct device *dev, bool set, enum ieee802154_filter_type type,
		      const struct ieee802154_filter *filter)
{
	...
	if (type == IEEE802154_FILTER_TYPE_IEEE_ADDR) {
		return tl521x_set_ieee_addr(dev, filter->ieee_addr);
	} else if (type == IEEE802154_FILTER_TYPE_SHORT_ADDR) {
		return tl521x_set_short_addr(dev, filter->short_addr);
	} else if (type == IEEE802154_FILTER_TYPE_PAN_ID) {
		return tl521x_set_pan_id(dev, filter->pan_id);
	}
...
RAM_CODE_SECTION_IEEE802154
static int tl521x_set_txpower(const struct device *dev, int16_t dbm)
{
	struct tl521x_data *tlx = dev->data;
...
#ifdef CONFIG_IEEE802154_TL521X_BLE_COEXIST
_attribute_ram_code_ void tl521x_rf_tx_is_sending(void)
{
	const struct device *dev = DEVICE_DT_INST_GET(0);
	struct tl521x_data *tlx = dev->data;

	if (tl521x_rf_zigbee_tx_is_sending == TL521X_RF_ZIGBEE_TX_IS_SENDING) {
		tl521x_rf_zigbee_tx_is_sending = TL521X_RF_ZIGBEE_TX_IS_STOPPED;
		/* release tx semaphore */
		k_sem_give(&tlx->tx_wait);
...
RAM_CODE_SECTION_IEEE802154
static int tl521x_start(const struct device *dev)
{
	return tl521x_start_radio(dev->data);
}

/* API implementation: stop */
RAM_CODE_SECTION_IEEE802154
static int tl521x_stop(const struct device *dev)
{
	return tl521x_stop_radio(dev->data);
}

#if defined CONFIG_IEEE802154_TL521X_OPTIMIZATION && CONFIG_IEEE802154_TL521X_OPTIMIZATION
RAM_CODE_SECTION_IEEE802154
static void tl521x_wfi_direct(uint32_t time_ms)
{
...
RAM_CODE_SECTION_IEEE802154
static int tl521x_tx(const struct device *dev, enum ieee802154_tx_mode mode, struct net_pkt *pkt,
		  struct net_buf *frag)
{
	ARG_UNUSED(pkt);
	...
	tl521x_rf_zigbee_tx_is_sending = TL521X_RF_ZIGBEE_TX_IS_SENDING;
	/* unlock interrupts */
	irq_unlock(r);
#endif /* CONFIG_IEEE802154_TL521X_BLE_COEXIST */
	...
	tl521x_ieee802154_frame_parse(frag->data, frag->len, &frame);
	...
	do {
		...
		if (net_pkt_ieee802154_mac_hdr_rdy(pkt)) {
			if (frame.payload_ie &&
			    tl521x_ieee802154_ie_csl_search(frame.payload, frame.payload_len)) {
				enc_rounds = 2;
			} else {
		...
		const uint8_t *key = tl521x_mac_keys_get(tlx->mac_keys, key_id);
		...
		if (enc_rounds == 1) {
			tl521x_mac_keys_frame_cnt_inc(tlx->mac_keys, key_id);
		}

		uint8_t *frame_cnt =
			(uint8_t *)&frame.sec_header[IEEE802154_FRAME_LENGTH_SEC_HEADER];
		uint32_t fc = tl521x_mac_keys_frame_cnt_get(tlx->mac_keys, key_id);
		...
		case IEEE802154_FRAME_SECCTRL_SEC_LEVEL_7:
			do {
				...
				if (tlx->csl_period > 0 && frame.payload_ie) {
					if (private_data) {
						ie_csl_pos = tl521x_ieee802154_ie_csl_search(
				...
				if (frame.payload_ie) {
					...
					if (private_data) {
						private_data =
							(uint8_t *)tl521x_ieee802154_get_data(
								private_data,
...
#ifdef CONFIG_OPENTHREAD_CSL_RECEIVER
					if (i == enc_rounds - 1) {
						tl521x_ieee802154_ie_csl_commit_at(
							ie_csl_pos, tlx->csl_period,
							tl521x_get_csl_phase(tlx));
					}
#endif /* CONFIG_OPENTHREAD_CSL_RECEIVER */
					if (!ieee802154_tl521x_crypto_encrypt(
						    key, src_addr,
						    tl521x_mac_keys_frame_cnt_get(tlx->mac_keys,
									       key_id),
						    sec_level, open_data,
	...
	if (
		...
		/* prepare tx buffer */
		tl521x_set_tx_payload(dev, frag->data, frag->len);
		...
		if (mode == IEEE802154_TX_MODE_TXTIME_CCA) {
			...
			delay_us(CONFIG_IEEE802154_TL521X_SET_TXRX_DELAY_US);
		}
#else
		delay_us(CONFIG_IEEE802154_TL521X_SET_TXRX_DELAY_US);
#endif /* CONFIG_NET_PKT_TIMESTAMP && CONFIG_NET_PKT_TXTIME */
	...
	if (isThreadCommissioned == true) {
		tl521x_wfi_direct(800);
	}
#endif /* CONFIG_IEEE802154_TL521X_OPTIMIZATION */
	if (k_sem_take(&tlx->tx_wait, K_MSEC(TL521X_TX_WAIT_TIME_MS)) != 0) {
		rf_set_rxmode();
		status = -EIO;
	} else {
	...
	if (!status && tlx->ack_handler_en) {
		{
			if (k_sem_take(&tlx->ack_wait, K_MSEC(TL521X_ACK_WAIT_TIME_MS)) != 0) {
				tlx->ack_handler_en = false;
				status = -ENOMSG;
			}
		}
...
static int tl521x_configure(const struct device *dev, enum ieee802154_config_type type,
	...
	case IEEE802154_CONFIG_ACK_FPB:
		if (config->ack_fpb.addr) {
			if (config->ack_fpb.enabled) {
				tl521x_src_match_table_add(tlx->src_match_table,
							   config->ack_fpb.addr,
							   config->ack_fpb.extended);
			} else {
				tl521x_src_match_table_remove(tlx->src_match_table,
							   config->ack_fpb.addr,
							   config->ack_fpb.extended);
			}
		} else if (!config->ack_fpb.enabled) {
			tl521x_src_match_table_remove_group(tlx->src_match_table,
							 config->ack_fpb.extended);
		} else {
			result = -ENOTSUP;
		}
	...
	case IEEE802154_CONFIG_ENH_ACK_HEADER_IE: {
		...
		if (!config->ack_ie.purge_ie) {
			if (config->ack_ie.header_ie && config->ack_ie.header_ie->length) {
				tl521x_enh_ack_table_add(tlx->enh_ack_table, short_addr, ext_addr,
						      config->ack_ie.header_ie);
			} else {
				tl521x_enh_ack_table_remove(tlx->enh_ack_table, short_addr,
							   ext_addr);
			}

		} else {
			tl521x_enh_ack_table_remove(tlx->enh_ack_table, short_addr, ext_addr);
		}
	} break;
#endif
	default:
...
static int tl521x_attr_get(const struct device *dev, enum ieee802154_attr attr,
...
static uint8_t tl521x_get_sch_acc(const struct device *dev)
{
	ARG_UNUSED(dev);

	return CONFIG_IEEE802154_TL521X_DELAY_TRX_ACC;
}

/* IEEE802154 driver APIs structure */
static struct ieee802154_radio_api tl521x_radio_api = {
	.iface_api.init = tl521x_iface_init,
	.get_capabilities = tl521x_get_capabilities,
	.cca = tl521x_cca,
	.set_channel = tl521x_set_channel,
	.filter = tl521x_filter,
	.set_txpower = tl521x_set_txpower,
	.start = tl521x_start,
	.stop = tl521x_stop,
	.tx = tl521x_tx,
	.ed_scan = tl521x_ed_scan,
	.configure = tl521x_configure,
	.attr_get = tl521x_attr_get,
	.get_sch_acc = tl521x_get_sch_acc,
};
...
NET_DEVICE_DT_INST_DEFINE(0, tl521x_init, NULL, &data, NULL, CONFIG_IEEE802154_TL521X_INIT_PRIO,
			  &tl521x_radio_api, L2, L2_CTX_TYPE, MTU);
#else
DEVICE_DT_INST_DEFINE(0, tl521x_init, NULL, &data, NULL, POST_KERNEL,
		      CONFIG_IEEE802154_TL521X_INIT_PRIO, &tl521x_radio_api);
#endif
