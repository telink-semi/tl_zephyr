...
static inline void net_pkt_set_timestamp_ns(struct net_pkt *pkt, uint64_t nsec)
{
	struct net_ptp_time ts = {
		.second = nsec / NSEC_PER_SEC,
		.nanosecond = nsec % NSEC_PER_SEC
	};

	net_pkt_set_timestamp(pkt, &ts);
}

static inline uint64_t net_pkt_timestamp_ns(struct net_pkt *pkt)
{
...
static void telink_rcp_zb_rx(const struct spinel_frame_data *frame, const void *ctx)
{
	...
	do {
		...
		net_pkt_set_ieee802154_rssi(rx_pkt, frame->rx.rssi);
	...
	if (!result && frame.data && frame.data_length) {
		...
		do {
			...
			net_pkt_set_timestamp_ns(ack_pkt, (frame.rx.timestamp - time_offset) *
								  NSEC_PER_USEC);
#endif /* CONFIG_NET_PKT_TIMESTAMP */
			net_pkt_set_ieee802154_rssi(ack_pkt, frame.rx.rssi);
				   ...
				   const struct ieee802154_config *config)
{
	...
	case IEEE802154_CONFIG_ENH_ACK_HEADER_IE:
		if (config->ack_ie.data) {
			struct spinel_link_metrics link_metrics = {};
			uint8_t len = config->ack_ie.data[0] & 0x7f;
			uint8_t *vendor_data = (uint8_t *)&config->ack_ie.data[2];
	...
	case IEEE802154_CONFIG_MAC_KEYS: {
		struct spinel_mac_keys mac_keys = {
			.key_mode = config->mac_keys[0].key_value
					    ...
					    : 0,
			.key_id = config->mac_keys[0].key_value ? config->mac_keys[1].key_index : 0,
		};
...
static uint64_t telink_rcp_zb_get_time(const struct device *dev)
{
	LOG_DBG("%s", __func__);
