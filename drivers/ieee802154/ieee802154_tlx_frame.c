 ...
 */
ALWAYS_INLINE static void tlx_ieee802154_frame_parse(const uint8_t *buf, size_t bul_len,
 ...
 */
ALWAYS_INLINE static bool tlx_ieee802154_frame_build(const struct ieee802154_frame *frame,
...
ALWAYS_INLINE static const uint8_t *tlx_ieee802154_get_data(const uint8_t *payload,
							    size_t payload_len)
{
...
ALWAYS_INLINE static uint8_t *tlx_ieee802154_ie_csl_search(const uint8_t *payload,
							   size_t payload_len)
{
	uint8_t *result = NULL;
	size_t pos = 0; /* current buffer position */
...
ALWAYS_INLINE static void tlx_ieee802154_ie_csl_commit_at(uint8_t *csl_pos, uint16_t aCslPeriod,
							  uint16_t aCslPhase)
{
	if (csl_pos) {
...
ALWAYS_INLINE static bool tlx_ieee802154_ie_csl_commit(uint8_t *payload, size_t payload_len,
						       uint16_t aCslPeriod, uint16_t aCslPhase)
{
	bool result = false;
	uint8_t *csl_data = tlx_ieee802154_ie_csl_search(payload, payload_len);

	if (csl_data) {
		tlx_ieee802154_ie_csl_commit_at(csl_data, aCslPeriod, aCslPhase);
		result = true;
	}
	return result;
}
...
ieee802154_tlx_crypto_ecb(const uint8_t key[IEEE802154_CRYPTO_LENGTH_AES_BLOCK],
...
ALWAYS_INLINE static void
ieee802154_tlx_crypto_key(struct ieee802154_crypto_ctx *ctx,
...
ALWAYS_INLINE static void
ieee802154_tlx_crypto_nonce(struct ieee802154_crypto_ctx *ctx,
...
ALWAYS_INLINE static void ieee802154_tlx_crypto_start(struct ieee802154_crypto_ctx *ctx,
						      ...
						      uint8_t tag_len)
{
	...
	ieee802154_tlx_crypto_ecb(ctx->key, ctx->blk, ctx->blk);
	ctx->blk_len = 0;
...
ALWAYS_INLINE static void ieee802154_tlx_crypto_header(struct ieee802154_crypto_ctx *ctx,
						       const uint8_t *open)
{
...
ALWAYS_INLINE static void ieee802154_tlx_crypto_payload(struct ieee802154_crypto_ctx *ctx,
							...
							uint8_t *cipher)
{
	...
	for (uint8_t i = 0; i < ctx->priv_len; i++) {
		if (ctr_len == sizeof(ctx->ctr)) {
			...
			ieee802154_tlx_crypto_ecb(ctx->key, ctx->ctr, ctx->ctr_pad);
			ctr_len = 0;
		}
		uint8_t byte;
...
ALWAYS_INLINE static void ieee802154_tlx_crypto_finish(struct ieee802154_crypto_ctx *ctx,
						       uint8_t *tag)
{
	ieee802154_tlx_crypto_ecb(ctx->key, ctx->ctr, ctx->ctr_pad);
...
ALWAYS_INLINE static bool ieee802154_tlx_crypto_check(struct ieee802154_crypto_ctx *ctx,
						      const uint8_t *tag)
{
	bool result = true;
...
ieee802154_tlx_crypto_encrypt(const uint8_t key[IEEE802154_CRYPTO_LENGTH_AES_BLOCK],
			      const uint8_t ext_addr[IEEE802154_FRAME_LENGTH_ADDR_EXT],
			      ...
			      uint8_t *frame_cipher, uint8_t *frame_mic, uint8_t frame_mic_len)
{
	...
	if (key && ext_addr && frame_open && frame_open_len && frame_mic && frame_mic_len) {
		...
		ieee802154_tlx_crypto_key(&ctx, key);
		ieee802154_tlx_crypto_nonce(&ctx, ext_addr, frame_cnt, frame_sec_level);
		ieee802154_tlx_crypto_start(&ctx, frame_open_len, frame_plain_len, frame_mic_len);
		ieee802154_tlx_crypto_header(&ctx, frame_open);
		ieee802154_tlx_crypto_payload(&ctx, true, (uint8_t *)frame_plain, frame_cipher);
		ieee802154_tlx_crypto_finish(&ctx, frame_mic);
		result = true;
	}
...
ALWAYS_INLINE static bool
ieee802154_tlx_crypto_decrypt(const uint8_t key[IEEE802154_CRYPTO_LENGTH_AES_BLOCK],
			      const uint8_t ext_addr[IEEE802154_FRAME_LENGTH_ADDR_EXT],
			      uint32_t frame_cnt, uint8_t frame_sec_level,
			      const uint8_t *frame_open, uint8_t frame_open_len,
			      ...
			      const uint8_t *frame_mic, uint8_t frame_mic_len, uint8_t *frame_plain)
{
	...
	if (key && ext_addr && frame_open && frame_open_len && frame_mic && frame_mic_len) {
		...
		ieee802154_tlx_crypto_key(&ctx, key);
		ieee802154_tlx_crypto_nonce(&ctx, ext_addr, frame_cnt, frame_sec_level);
		ieee802154_tlx_crypto_start(&ctx, frame_open_len, frame_cipher_len, frame_mic_len);
		ieee802154_tlx_crypto_header(&ctx, frame_open);
		ieee802154_tlx_crypto_payload(&ctx, false, frame_plain, (uint8_t *)frame_cipher);
		result = ieee802154_tlx_crypto_check(&ctx, frame_mic);
	}
