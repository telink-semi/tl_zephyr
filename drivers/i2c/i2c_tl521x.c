...
#define DT_DRV_COMPAT telink_tl521x_i2c
...
struct i2c_tl521x_cfg {
	uint32_t bitrate;
	const struct pinctrl_dev_config *pcfg;
};

/* I2C data structure */
struct i2c_tl521x_data {
	struct k_sem mutex;
};

/* API implementation: configure */
static int i2c_tl521x_configure(const struct device *dev, uint32_t dev_config)
{
	ARG_UNUSED(dev);
...
static int i2c_tl521x_transfer(const struct device *dev,
			       struct i2c_msg *msgs,
			       uint8_t num_msgs,
			       uint16_t addr)
{
	int status = 0;
	uint8_t send_stop = 0;
	struct i2c_tl521x_data *data = dev->data;
...
static int i2c_tl521x_init(const struct device *dev)
{
	int status = 0;
	const struct i2c_tl521x_cfg *cfg = dev->config;
	struct i2c_tl521x_data *data = dev->data;
	...
	status = i2c_tl521x_configure(dev, dev_config);
	if (status != 0) {
...
static int i2c_tl521x_pm_action(const struct device *dev, enum pm_device_action action)
{
	const struct i2c_tl521x_cfg *cfg = dev->config;
	const pinctrl_soc_pin_t *i2cPinsMux = cfg->pcfg->states->pins;
	uint32_t dev_config = (I2C_MODE_CONTROLLER | i2c_map_dt_bitrate(cfg->bitrate));
	...
	case PM_DEVICE_ACTION_RESUME:
	{
		...
		if (pm_has_resumed_from_deep_sleep_retention()) {
			i2c_tl521x_configure(dev, dev_config);
		}
...
PM_DEVICE_DT_INST_DEFINE(0, i2c_tl521x_pm_action);
#endif /* CONFIG_PM_DEVICE */

/* I2C driver APIs structure */
static const struct i2c_driver_api i2c_tl521x_api = {
	.configure = i2c_tl521x_configure,
	.transfer = i2c_tl521x_transfer,
};
...
#define I2C_TL521X_INIT(inst)					      \
								      \
	PINCTRL_DT_INST_DEFINE(inst);				      \
								      \
	static struct i2c_tl521x_data i2c_tl521x_data_##inst;	      \
								      \
	static struct i2c_tl521x_cfg i2c_tl521x_cfg_##inst = {	      \
		.bitrate = DT_INST_PROP(inst, clock_frequency),	      \
		.pcfg = PINCTRL_DT_INST_DEV_CONFIG_GET(inst),	      \
	};							      \
								      \
	I2C_DEVICE_DT_INST_DEFINE(inst, i2c_tl521x_init,		      \
				  PM_DEVICE_DT_INST_GET(0),			      \
				  &i2c_tl521x_data_##inst,		      \
				  &i2c_tl521x_cfg_##inst,		      \
				  POST_KERNEL,			      \
				  CONFIG_I2C_INIT_PRIORITY,	      \
				  &i2c_tl521x_api);

DT_INST_FOREACH_STATUS_OKAY(I2C_TL521X_INIT)
