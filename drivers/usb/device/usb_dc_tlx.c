...
#define USBD_TLX_IRQN_BY_IDX(idx)	  DT_INST_IRQN_BY_IDX(0, idx)
...
K_MSGQ_DEFINE(usbd_event_msgq, sizeof(struct usbd_event),
	CONFIG_USB_TLX_EVT_QUEUE_SIZE, sizeof(uint32_t));
static void usbd_work_handler(struct k_msgq *event_msgq);
static K_THREAD_DEFINE(usbd_tlx, CONFIG_USB_TLX_THREAD_STACK_SIZE,
	usbd_work_handler, &usbd_event_msgq, NULL, NULL, CONFIG_USB_TLX_THREAD_PRIORITY, 0, 0);
...
static void usb_irq_setup_handler(void)
{
	...
	if (get_usbd_ctx()->suspend) {
		...
		riscv_plic_irq_enable(USBD_TLX_IRQN_BY_IDX(5));
...
static void usb_irq_suspend(void)
{
	...
	riscv_plic_irq_disable(USBD_TLX_IRQN_BY_IDX(5));
	if (!get_usbd_ctx()->suspend) {
...
static int usb_irq_init(void)
{
	...
	riscv_plic_irq_enable(USBD_TLX_IRQN_BY_IDX(0));
	riscv_plic_set_priority(USBD_TLX_IRQN_BY_IDX(0), USBD_TLX_IRQ_PRIORITY_BY_IDX(0));

	IRQ_CONNECT(USBD_TLX_IRQN_BY_IDX(1), USBD_TLX_IRQ_PRIORITY_BY_IDX(1), usb_irq_data, 0, 0);
	riscv_plic_irq_enable(USBD_TLX_IRQN_BY_IDX(1));
	riscv_plic_set_priority(USBD_TLX_IRQN_BY_IDX(1), USBD_TLX_IRQ_PRIORITY_BY_IDX(1));

	IRQ_CONNECT(USBD_TLX_IRQN_BY_IDX(2), USBD_TLX_IRQ_PRIORITY_BY_IDX(2), usb_irq_status, 0, 0);
	riscv_plic_irq_enable(USBD_TLX_IRQN_BY_IDX(2));
	riscv_plic_set_priority(USBD_TLX_IRQN_BY_IDX(2), USBD_TLX_IRQ_PRIORITY_BY_IDX(2));

	IRQ_CONNECT(USBD_TLX_IRQN_BY_IDX(4), USBD_TLX_IRQ_PRIORITY_BY_IDX(4), usb_irq_eps, 0, 0);
	riscv_plic_irq_enable(USBD_TLX_IRQN_BY_IDX(4));
	riscv_plic_set_priority(USBD_TLX_IRQN_BY_IDX(4), USBD_TLX_IRQ_PRIORITY_BY_IDX(4));

	IRQ_CONNECT(USBD_TLX_IRQN_BY_IDX(5), USBD_TLX_IRQ_PRIORITY_BY_IDX(5), usb_irq_suspend, 0,
		    0);
	riscv_plic_irq_enable(USBD_TLX_IRQN_BY_IDX(5));
	riscv_plic_set_priority(USBD_TLX_IRQN_BY_IDX(5), USBD_TLX_IRQ_PRIORITY_BY_IDX(5));
	get_usbd_ctx()->suspend_ignore = true;

	IRQ_CONNECT(USBD_TLX_IRQN_BY_IDX(6), USBD_TLX_IRQ_PRIORITY_BY_IDX(6), usb_irq_reset, 0, 0);
	riscv_plic_irq_enable(USBD_TLX_IRQN_BY_IDX(6));
	riscv_plic_set_priority(USBD_TLX_IRQN_BY_IDX(6), USBD_TLX_IRQ_PRIORITY_BY_IDX(6));
