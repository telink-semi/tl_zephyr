...
static bool spi_tlx_config_cs(const struct device *dev, const struct spi_config *config)
{
	...
	if (spi_cs_is_gpio(config)) {
		...
		spi_tlx_hw_cs_disable(tlx_config);
		return true;
	}
	...
	for (int cs_id = 0; cs_id < CHIP_SELECT_COUNT; cs_id++) {
		/* get cs pin defined in device tree */
		cs_pin = tlx_config->cs_pin[cs_id];
...
static uint32_t spi_tlx_get_txrx_len(const struct spi_buf_set *tx_bufs,
...
_attribute_ram_code_sec_ static void spi_tlx_txrx(const struct device *dev, uint32_t len)
{
	...
	for (int i = 0; i < len; i = i + chunk_size) {
		...
		/* write bytes */
		spi_tlx_tx(cfg->peripheral_id, ctx, chunk_size);

		/* read bytes */
		if (len <= SPI_WR_RD_CHUNK_SIZE_MAX) {
			/* read all bytes if len is less than chunk size */
			spi_tlx_rx(cfg->peripheral_id, ctx, chunk_size);
		} else if (i == 0) {
			/* head, read 1 byte less than is sent */
			spi_tlx_rx(cfg->peripheral_id, ctx, chunk_size - 1);
...
static bool spi_tlx_is_config_supported(const struct spi_config *config,
					struct spi_tlx_cfg *tlx_config)
{
...
static int spi_tlx_config(const struct device *dev, const struct spi_config *config)
{
	...
	if (!spi_tlx_is_config_supported(config, tlx_config)) {
		return -ENOTSUP;
	}

	/* config slave selection (CS): hw or sw */
	if (!spi_tlx_config_cs(dev, config)) {
		return -ENOTSUP;
	}
	...
	if (IS_ENABLED(CONFIG_SPI_EXTENDED_MODES)) {
		...
		if (lines == SPI_LINES_SINGLE) {
			spi_set_io_mode(tlx_config->peripheral_id, SPI_SINGLE_MODE);
	...
	for (uint8_t i = 0; i < tlx_config->pcfg->states->pin_cnt; i++) {
		gpio_input_en(TLX_PINMUX_GET_PIN(*pins++));
	}

	/* configure pins */
	status = pinctrl_apply_state(tlx_config->pcfg, PINCTRL_STATE_DEFAULT);
	if (status < 0) {
	...
	/* save context config */
	tlx_data->ctx.config = config;

	return 0;
}

/* API implementation: init */
static int spi_tlx_init(const struct device *dev)
{
	int err;
...
static struct spi_driver_api spi_tlx_api = {
	.transceive = spi_tlx_transceive,
	.release = spi_tlx_release,
#ifdef CONFIG_SPI_ASYNC
	.transceive_async = spi_tlx_transceive_async,
#endif /* CONFIG_SPI_ASYNC */
};
