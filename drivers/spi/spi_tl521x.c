 ...
 */
#define spi_read  __hal_spi_read_unused
#define spi_write __hal_spi_write_unused
#include "spi.h"
/* Declare hal_spi_read/hal_spi_write with __asm__ labels so calls resolve to
 * the HAL's original spi_read/spi_write symbols at link time, without
 * conflicting with Zephyr's spi_read/spi_write (included later).
 */
extern drv_api_status_e hal_spi_read(spi_sel_e spi_sel, unsigned char *data,
...
struct spi_tl521x_cfg {
	uint8_t peripheral_id;
	gpio_pin_e cs_pin[CHIP_SELECT_COUNT];
	const struct pinctrl_dev_config *pcfg;
};
#define SPI_CFG(dev) ((struct spi_tl521x_cfg *)((dev)->config))

/* SPI data structure */
struct spi_tl521x_data {
	struct spi_context ctx;
};
#define SPI_DATA(dev) ((struct spi_tl521x_data *)((dev)->data))

/* disable hardware cs flow control */
static void spi_tl521x_hw_cs_disable(const struct spi_tl521x_cfg *config)
{
	gpio_pin_e pin = 0;
...
static bool spi_tl521x_config_cs(const struct device *dev, const struct spi_config *config)
{
	gpio_pin_e cs_pin = 0;
	const struct spi_tl521x_cfg *tl521x_config = SPI_CFG(dev);

	/* software flow control */
	if (spi_cs_is_gpio(config)) {
		/* disable all hardware CS pins */
		spi_tl521x_hw_cs_disable(tl521x_config);
		return true;
	}
	...
	for (int cs_id = 0; cs_id < CHIP_SELECT_COUNT; cs_id++) {
		/* get cs pin defined in device tree */
		cs_pin = tl521x_config->cs_pin[cs_id];
...
static uint32_t spi_tl521x_get_txrx_len(const struct spi_buf_set *tx_bufs,
...
/* process tx data */
_attribute_ram_code_sec_ static void spi_tl521x_tx(uint8_t peripheral_id, struct spi_context *ctx,
						   uint8_t len)
{
	uint8_t tx;
...
/* process rx data */
_attribute_ram_code_sec_ static void spi_tl521x_rx(uint8_t peripheral_id, struct spi_context *ctx,
						   uint8_t len)
{
	uint8_t rx = 0;
...
_attribute_ram_code_sec_ static void spi_tl521x_txrx(const struct device *dev, uint32_t len)
{
	unsigned int chunk_size = SPI_WR_RD_CHUNK_SIZE_MAX;
	struct spi_tl521x_cfg *cfg = SPI_CFG(dev);
	...
	for (int i = 0; i < len; i = i + chunk_size) {
		...
		/* write bytes */
		spi_tl521x_tx(cfg->peripheral_id, ctx, chunk_size);

		/* read bytes */
		if (len <= SPI_WR_RD_CHUNK_SIZE_MAX) {
			/* read all bytes if len is less than chunk size */
			spi_tl521x_rx(cfg->peripheral_id, ctx, chunk_size);
		} else if (i == 0) {
			/* head, read 1 byte less than is sent */
			spi_tl521x_rx(cfg->peripheral_id, ctx, chunk_size - 1);
...
static bool spi_tl521x_is_config_supported(const struct spi_config *config,
					   struct spi_tl521x_cfg *tl521x_config)
{
...
/* SPI configuration */
static int spi_tl521x_config(const struct device *dev, const struct spi_config *config)
{
	int status = 0;
	spi_mode_type_e mode = SPI_MODE0;
	struct spi_tl521x_cfg *tl521x_config = SPI_CFG(dev);
	struct spi_tl521x_data *tl521x_data = SPI_DATA(dev);
	const pinctrl_soc_pin_t *pins = tl521x_config->pcfg->states->pins;
	uint8_t clk_src = sys_clk.pll_clk;

	/* check for unsupported configuration */
	if (!spi_tl521x_is_config_supported(config, tl521x_config)) {
		return -ENOTSUP;
	}

	/* config slave selection (CS): hw or sw */
	if (!spi_tl521x_config_cs(dev, config)) {
		return -ENOTSUP;
	}
	...
	/* init SPI master */
	spi_master_init(tl521x_config->peripheral_id, clk_src * 1000000 / config->frequency, mode);
	spi_master_config(tl521x_config->peripheral_id, SPI_NORMAL);
	...
	if (IS_ENABLED(CONFIG_SPI_EXTENDED_MODES)) {
		...
		if (lines == SPI_LINES_SINGLE) {
			spi_set_io_mode(tl521x_config->peripheral_id, SPI_SINGLE_MODE);
		} else if (lines == SPI_LINES_DUAL) {
			spi_set_io_mode(tl521x_config->peripheral_id, SPI_DUAL_MODE);
		} else if (lines == SPI_LINES_QUAD) {
			spi_set_io_mode(tl521x_config->peripheral_id, SPI_QUAD_MODE);
		}
	}
	/* The GPIO need to be configured as input to support alternative function */
	for (uint8_t i = 0; i < tl521x_config->pcfg->states->pin_cnt; i++) {
		gpio_input_en(TL521X_PINMUX_GET_PIN(*pins++));
	}

	/* configure pins */
	status = pinctrl_apply_state(tl521x_config->pcfg, PINCTRL_STATE_DEFAULT);
	if (status < 0) {
	...
	/* save context config */
	tl521x_data->ctx.config = config;

	return 0;
}

/* API implementation: init */
static int spi_tl521x_init(const struct device *dev)
{
	int err;
	struct spi_tl521x_data *data = SPI_DATA(dev);
...
static int spi_tl521x_transceive(const struct device *dev, const struct spi_config *config,
				 const struct spi_buf_set *tx_bufs,
				 const struct spi_buf_set *rx_bufs)
{
	int status = 0;
	struct spi_tl521x_data *data = SPI_DATA(dev);
	uint32_t txrx_len = spi_tl521x_get_txrx_len(tx_bufs, rx_bufs);

	/* set configuration */
	status = spi_tl521x_config(dev, config);
	if (status) {
		return status;
	}
...
static int spi_tl521x_transceive_async(const struct device *dev, const struct spi_config *config,
				       const struct spi_buf_set *tx_bufs,
...
static int spi_tl521x_release(const struct device *dev, const struct spi_config *config)
{
	struct spi_tl521x_data *data = SPI_DATA(dev);
...
static struct spi_driver_api spi_tl521x_api = {
	.transceive = spi_tl521x_transceive,
	.release = spi_tl521x_release,
#ifdef CONFIG_SPI_ASYNC
	.transceive_async = spi_tl521x_transceive_async,
#endif /* CONFIG_SPI_ASYNC */
};

/* SPI driver registration */
#define SPI_TL521X_INIT(inst)                                                                      \
                                                                                                   \
	PINCTRL_DT_INST_DEFINE(inst);                                                              \
                                                                                                   \
	static struct spi_tl521x_data spi_tl521x_data_##inst = {                                   \
		SPI_CONTEXT_INIT_LOCK(spi_tl521x_data_##inst, ctx),                                \
		SPI_CONTEXT_INIT_SYNC(spi_tl521x_data_##inst, ctx),                                \
		SPI_CONTEXT_CS_GPIOS_INITIALIZE(DT_DRV_INST(inst), ctx)};                          \
                                                                                                   \
	static struct spi_tl521x_cfg spi_tl521x_cfg_##inst = {                                     \
		...
		.pcfg = PINCTRL_DT_INST_DEV_CONFIG_GET(inst),                                      \
                                                                                                   ...
                                                                                                   \
	DEVICE_DT_INST_DEFINE(inst, spi_tl521x_init, NULL, &spi_tl521x_data_##inst, \
			      &spi_tl521x_cfg_##inst, POST_KERNEL, CONFIG_SPI_INIT_PRIORITY, \
			      &spi_tl521x_api);

DT_INST_FOREACH_STATUS_OKAY(SPI_TL521X_INIT)
