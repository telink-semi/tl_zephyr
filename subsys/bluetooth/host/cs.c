...
void bt_hci_le_cs_subevent_result_continue(struct net_buf *buf)
{
	...
	reassembly_buf->reassembled_result.header.subevent_done_status =
		evt->subevent_done_status;
...
void bt_hci_le_cs_test_end_complete(struct net_buf *buf)
{
	...
	struct cs_reassembly_buf *reassembly_buf = get_reassembly_buf(BT_HCI_LE_CS_TEST_CONN_HANDLE, false);
